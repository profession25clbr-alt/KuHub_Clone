...
export const DashboardAdmin: React.FC = () => {
  ...
  const iniciarComprobacion = async () => {
    try {
      ...
      const solicitudesSeleccionadas = estadoProceso.semanaSeleccionada
        ? solicitudesAceptadas.filter(s => s.semana === estadoProceso.semanaSeleccionada)
        : solicitudesAceptadas;
      
      if (solicitudesSeleccionadas.length === 0) {
        toast.warning('No hay solicitudes aceptadas para procesar');
        return;
      }
      ...
      if (estadoProceso.pedidoId) {
        try {
          await sincronizarSolicitudesPedidoService(
            ...
            solicitudesSeleccionadas.map((s) => s.id)
          );
        } catch (error) {
          logger.error('❌ Error al sincronizar solicitudes del pedido:', error);
        }
      }
